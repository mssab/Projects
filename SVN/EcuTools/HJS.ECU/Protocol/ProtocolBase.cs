 ...
 * $LastChangedBy: ksi $
 * $HeadURL: http://menden22/svn/devel/electronic/app_cs_win32_ecudiagmini/branch/EcuTools/HJS.ECU/Protocol/ProtocolBase.cs $
 * 
 * LastReviewDate: 
 * LastReviewRevision: 
 * LastReviewBy: 
...
namespace HJS.ECU.Protocol
{
    ...
    public abstract class ProtocolBase
    {
        ...
        public UInt16 GetLastChecksum()
        {
            if (mEcu != null)
            {
                return mEcu.Checksum;
            }
            else
            {
                return 0;
        ...
        public abstract Int16 GetEcuTemperatureOffset();

        /// <summary>Read report block from ecu</summary>
        /// <param name="Report">Reference to target block</param>
        /// <returns>0 on success, else see ReturnValue</returns>
        public abstract ReturnValue ReadReportBlock(ref Block Report);
        ...
        public bool IsUpdateableTo(string Version)
        {
            if (mInfo != null)
            {
                return mFirmware.IsUpdateableTo(Version);
            }
            else
            {
                return false;
        ...
        public static Block.BlockId GetLanguageBlockId(LanguageId lId)
        {
            ...
            switch (lId)
            {
                ...
                case LanguageId.Polish:
                    ret = Block.BlockId.IdLngPL;
                    break;
                case LanguageId.Dutch:
                    ret = Block.BlockId.IdLngNL;
                    break;
                case LanguageId.Turkish:
                    ret = Block.BlockId.IdLngTK;
                    break;
            }
            return ret;
        }
        ...
        /// <param name="target">Output byte array</param>
        /// <returns>0 on sucess, else see ReturnValue</returns>
        public abstract ReturnValue ReadUpdatables(out byte[] target);
