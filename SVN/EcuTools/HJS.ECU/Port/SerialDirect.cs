 ...
 * $LastChangedBy: ksi $
 * $HeadURL: http://menden22/svn/devel/electronic/app_cs_win32_ecudiagmini/branch/EcuTools/HJS.ECU/Port/SerialDirect.cs $
 * 
 * LastReviewDate: 
 * LastReviewRevision: 
 * LastReviewBy: 
...
namespace HJS.ECU.Port
{
    ...
    public class SerialDirect : Comm, IDisposable
    {
        ...
        public override ReturnValue Connect()
        {
            ...
            // Set Timeouts
            mComPort.WriteTimeout = 1000;
            mComPort.ReadTimeout = 1000;
            // Set Handshake
        ...
        private ReturnValue readHeader()
        {
            ...
            try
            {
                ...
                // Read status 
                ReadBuffer = mComPort.ReadByte();
                Console.WriteLine("< " + mOrder + "(" + (StatusByte)ReadBuffer + ")");
                if ((byte)ReadBuffer != (byte)StatusByte.NoError)
                {
                    switch ((StatusByte)ReadBuffer)
                    {
                        ...
                        case StatusByte.ModeFailed:
                            return ReturnValue.ComOrderModeFailed;
