 ...
 * Description: Configuration parameter block compatibility 10
 * 
 * $LastChangedDate: 2016-06-07 14:15:37 +0200 (Di, 07 Jun 2016) $
 * $LastChangedRevision: 103 $
 * $LastChangedBy: wtr $
 * $HeadURL: http://menden22/svn/devel/electronic/app_cs_win32_ecudiagmini/branch/EcuTools/HJS.ECU/Parameter/Configuration10_Block.cs $
 * 
 * LastReviewDate: 
 * LastReviewRevision: 
 * LastReviewBy: 
...
namespace HJS.ECU.Parameter
{
    ...
    public partial class Configuration10_Block : ConfigurationBlock
    {
        /// <summary>Default constructor</summary>
        public Configuration10_Block()
        {
            Version = 10;
            DataSize = 4090;
        }
        ...
        public override ReturnValue Import(ref Block Source, bool KeepVersion)
        {
            ...
            if (_ret == ReturnValue.NoError && mBlockData != null)
            {
                ...
                for (int i = 0; i < mTaskAnzahl; i++)
                {
                    ...
                    if (!maTask[i].Import10(GetTaskOffset(i), ref mBlockData))
                    {
                        _ret = ReturnValue.SizeMismatch;
                    }
                }
        ...
        public override bool CheckTaskOffset(out String ResultString, out UInt32 UsedBytes)
        {
            ...
            UInt32 TaskOffset = 0;
            UInt32 TaskBytes = 0;
            UsedBytes = 6 + 9 + 75 + 175 + 236 + 195;
            // Block header(6) Verionen+Anzahl(9), TVT(25*3=75)
            // + taskerrors(25*7 = 175) + initvalues(236) + can2(195)
        ...
        public override UInt16 GetFixedOffset(TaskIdentifier task)
        {
            switch (task)
            {
                ...
                case TaskIdentifier.taskKommunikation:
                    return 0x4AB;   //wtr/jdr 2016-0, alle Änderungen bis Ende der Funktion...
        ...
        private ReturnValue Upgrade(Block OldBlock)
        {
            ...
            switch (OldBlock.Version)
            {
                case 8:
                    ...
                    break;
                case 9:
                    Source = new Configuration9_Block();
                    break;
                default:
            ...
            for (int i = 0; i < InitValueGetNumber(); i++)
            {
                ...
                if (!value_found)
                {
                    ...
                    else if (InitValueGetItemName(i) == "sDosi.ulAdditivtankinhalt")
                    {
                        InitValueSetItemValue(i, (double)800);
                    }
                    else if (InitValueGetItemName(i) == "sAddi.ulAddStandard")
                    {
                        InitValueSetItemValue(i, (double)20000);
                    }
                    else if (InitValueGetItemName(i) == "sStaudruck.fAirMassKFactor")
                    {
                        InitValueSetItemValue(i, (double)10);
                    }
                    else if (InitValueGetItemName(i) == "sMaf.fDisplacedVolume")
                    {
                        InitValueSetItemValue(i, (double)1);
                    }
                    else
                    {
            ...
            {
                for (int i = 0; i < SecondCanGetNumber(); i++)
                {
                    value_found = false;
                    for (int j = 0; j < Source.SecondCanGetNumber(); j++)
                    {
                        if (SecondCanGetItemName(i) == Source.SecondCanGetItemName(j))
                        {
                            SecondCanSetItemValue(i, Source.SecondCanGetItemValue(j));
                            value_found = true;
                        }
                    }
                    if (!value_found)
                    {
                        Console.WriteLine("Upgrade Error: Second Can not found: " + SecondCanGetItemValue(i));
                    }
                }
            }
            ...
            {
                maTask[i] = new TaskConfiguration(GetTaskIdentifier(i));
                maTask[i].Import10(GetFixedOffset(GetTaskIdentifier(i)), ref mBlockData);
                for (int t = 0; t < GetTaskItemNumber(i); t++)
                {
                        ...
                        for (int s = 0; s < Source.GetTaskItemNumber(i); s++)
                        {
                            if (GetTaskItemName(i, t) == Source.GetTaskItemName(i, s))
                            {
                                ...
                                if (posSource >= 0)
                                {
                                    ...
                                    {
                                        if ((GetTaskItemName(i, t) == "ulAschepulse") && (Source.Version < 9))
                                        {
                    ...
                    if (!value_found)
                    {
                        ...
                        //  Changes from 9 to 10
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskDosieren)
                            && (GetTaskItemName(i, t) == "ucEnableAdditiveLevelSensor"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskDosieren)
                            && (GetTaskItemName(i, t) == "uiADReserveUpperThreshold"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskDosieren)
                            && (GetTaskItemName(i, t) == "uiADReserveLowerThreshold"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)1024);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskDosieren)
                            && (GetTaskItemName(i, t) == "uiADReserveShortCircuit"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskDosieren)
                            && (GetTaskItemName(i, t) == "uiADReserveOpenCircuit"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)1024);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskDosieren)
                            && (GetTaskItemName(i, t) == "ucDualCanMailBox"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)255);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[8].ucPlausibilitaetscheckFlag"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[8].uiValueMax"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[8].uiValueMin"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[9].ucPlausibilitaetscheckFlag"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[9].uiValueMax"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[9].uiValueMin"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[10].ucPlausibilitaetscheckFlag"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[10].uiValueMax"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[10].uiValueMin"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[11].ucPlausibilitaetscheckFlag"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[11].uiValueMax"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "unsignedMesswertParameter[11].uiValueMin"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[8].ucPlausibilitaetscheckFlag"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[8].siValueMax"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[8].siValueMin"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[9].ucPlausibilitaetscheckFlag"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[9].siValueMax"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[9].siValueMin"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[10].ucPlausibilitaetscheckFlag"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[10].siValueMax"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[10].siValueMin"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[11].ucPlausibilitaetscheckFlag"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[11].siValueMax"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "signedMesswertParameter[11].siValueMin"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0xFFFF);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[8].ulMBID"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)-1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[8].ucDLC"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)8);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[8].eMRW"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)255);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[8].ucStartbit"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[8].fFaktor"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[8].iOffset"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[8].iPlausibel"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)30000);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[8].ucBitMask"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)16);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[8].ucSendRate"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)10);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[9].ulMBID"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)-1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[9].ucDLC"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)8);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[9].eMRW"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)255);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[9].ucStartbit"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[9].fFaktor"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[9].iOffset"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[9].iPlausibel"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)30000);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[9].ucBitMask"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)16);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[9].ucSendRate"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)10);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[10].ulMBID"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)-1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[10].ucDLC"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)8);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[10].eMRW"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)255);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[10].ucStartbit"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[10].fFaktor"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[10].iOffset"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[10].iPlausibel"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)30000);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[10].ucBitMask"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)16);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[10].ucSendRate"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)10);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[11].ulMBID"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)-1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[11].ucDLC"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)8);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[11].eMRW"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)255);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[11].ucStartbit"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[11].fFaktor"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[11].iOffset"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[11].iPlausibel"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)30000);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[11].ucBitMask"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)16);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskCanIn)
                            && (GetTaskItemName(i, t) == "saWert[11].ucSendRate"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)10);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskTankgeber)
                            && (GetTaskItemName(i, t) == "uiReferenceVoltage"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)12000);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskTankgeber)
                            && (GetTaskItemName(i, t) == "fVoltageFactor"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskRegenerieren)
                            && (GetTaskItemName(i, t) == "uiEntprellDauer"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskRegenerieren)
                            && (GetTaskItemName(i, t) == "ucResetNachSperrzeit"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)0);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskRegenerieren)
                        ...
                        && (GetTaskItemName(i, t) == "uiNotregUpm"))     //wtr/jdr 2016-01
                        {
                            maTask[i].SetItemValue(posTarget, (double)800);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskStaudruck)
                            && (GetTaskItemName(i, t) == "ulKFKFactor"))    
                        {
                            maTask[i].SetItemValue(posTarget, (double)-1);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskStaudruck)
                            && (GetTaskItemName(i, t) == "eTypeKFKFactor"))
                        {
                            maTask[i].SetItemValue(posTarget, (double)23);
                        }
                        else if ((GetTaskIdentifier(i) == TaskIdentifier.taskPostDiagnose)
                        && (GetTaskItemName(i, t) == "ucDeratingError"))    //wtr/jdr 2016-01
                        {
                            maTask[i].SetItemValue(posTarget, (double)255);
                        }
                        else
            ...
            // Dosieren.ulAdditivtankinhalt - > Init.sDosi.ulAdditivtankinhalt
            for (int i = 0; i < mTaskAnzahl; i++)
            {
                if (Source.GetTaskIdentifier(i) == TaskIdentifier.taskDosieren)
                {
                    posSource = Source.GetTaskItemPosition(i, "ulAdditivtankinhalt");
                    for (int j = 0; j < InitValueGetNumber(); j++)
                    {
                        if (InitValueGetItemName(j) == "sDosi.ulAdditivtankinhalt")
                        {
                            InitValueSetItemValue(j, Source.GetTaskItemValue(i, posSource));
                            Console.WriteLine("Upgrade Replace: Initvalue sDosi.ulAdditivtankinhalt set to " + InitValueGetItemValueString(j));
                        }
                    }
                }
            }
            // Additivieren.ulAddStandard - > Init.sAddi.ulAddStandard
            for (int i = 0; i < mTaskAnzahl; i++)
            {
                if (Source.GetTaskIdentifier(i) == TaskIdentifier.taskAdditivierung)
                {
                    posSource = Source.GetTaskItemPosition(i, "ulAddStandard");
                    for (int j = 0; j < InitValueGetNumber(); j++)
                    {
                        if (InitValueGetItemName(j) == "sAddi.ulAddStandard")
                        {
                            InitValueSetItemValue(j, Source.GetTaskItemValue(i, posSource));
                            Console.WriteLine("Upgrade Replace: Initvalue sAddi.ulAddStandard set to " + InitValueGetItemValueString(j));
                        }
                    }
                }
            }
            // Staudruck.fAirMassKFactor - > Init.sStaudruck.fAirMassKFactor
            for (int i = 0; i < mTaskAnzahl; i++)
            {
                if (Source.GetTaskIdentifier(i) == TaskIdentifier.taskStaudruck)
                {
                    posSource = Source.GetTaskItemPosition(i, "fAirMassKFactor");
                    for (int j = 0; j < InitValueGetNumber(); j++)
                    {
                        if (InitValueGetItemName(j) == "sStaudruck.fAirMassKFactor")
                        {
                            InitValueSetItemValue(j, Source.GetTaskItemValue(i, posSource));
                            Console.WriteLine("Upgrade Replace: Initvalue sStaudruck.fAirMassKFactor set to " + InitValueGetItemValueString(j));
                        }
                    }
                }
            }
            // MAF-Task.fDisplacedVolume - > Init.sMaf.fDisplacedVolume
            for (int i = 0; i < mTaskAnzahl; i++)
            {
                if (Source.GetTaskIdentifier(i) == TaskIdentifier.taskMassAirFlow)
                {
                    posSource = Source.GetTaskItemPosition(i, "fDisplacedVolume");
                    for (int j = 0; j < InitValueGetNumber(); j++)
                    {
                        if (InitValueGetItemName(j) == "sMaf.fDisplacedVolume")
                        {
                            InitValueSetItemValue(j, Source.GetTaskItemValue(i, posSource));
                            Console.WriteLine("Upgrade Replace: Initvalue sMaf.fDisplacedVolume set to " + InitValueGetItemValueString(j));
                        }
                    }
                }
            }
