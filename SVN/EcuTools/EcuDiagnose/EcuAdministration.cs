/*
 * Object: EcuDiagnose.EcuAdministration
 * Description: HJS-ECU admisistartion interface
 * 
 * $LastChangedDate: 2015-02-13 09:25:48 +0100 (Fr, 13 Feb 2015) $
 * $LastChangedRevision: 87 $
 * $LastChangedBy: ksi $
 * $HeadURL: http://menden22/svn/devel/electronic/app_cs_win32_ecudiagmini/branch/EcuTools/EcuDiagnose/EcuAdministration.cs $
 * 
 * LastReviewDate: 
 * LastReviewRevision: 
 * LastReviewBy: 
...
using HJS.ECU.Diagnostics;

namespace EcuDiagnose
{
    /// <summary>Administartion object for ECU</summary>
    public class EcuAdministration : IDisposable
    {
        private Diag mECU;
        ...
        private string[] mUpdatables;

        private AutoResetEvent mReadComplete;

        /// <summary>Accessors to raw file name w/o extension</summary>
        public string RawFileName
        {
            get { return mRawFileName; }
            set { mRawFileName = value; }
        }
        ...
        /// Read only</summary>
        public byte CompatibilityNumber
        {
            get
            {
                if (mECU == null) return 0;
                else return mECU.CompatibilityNumber;
            }
        }
        ...
        public EcuAdministration()
        {
            ...
            mLocalTime = false;
            mReadComplete = new AutoResetEvent(false);
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (disposing)
            {
                if (mReadComplete != null) mReadComplete.Close();
            }
            else
            {
            }
        ...
        public bool Connect(string portName, byte Language, bool LocalTime)
        {
            ...
            mECU.ChangePassword(BitConverter.GetBytes(Properties.Settings.Default.Key));
            ...
            if (ret)
            {
                ...
                mRawFileName = String.Format("{0}_{1}", mECU.SerialNumber, DateTime.Now.ToString("yyMMdd"));
            }
            else
            {
            ...
            mUpdatables = new string[mECU.GetNumberOfUpdatables()];
            return ret;
        }
        ...
        public void ReadAcquisition(string FileName, int FileExtensionIndex)
        {
            ...
            AcquiThread.Name = "ReadAcquisition";
            tellProgress(0);
            AcquiThread.Start();
        }
                            ...
                            if (ret == HJS.ReturnValue.NoError)
                            {
                                ...
                                if (ret == HJS.ReturnValue.NoError)
                                {
                                    ...
                                    if (ret == HJS.ReturnValue.NoError)
                                    {
                                        ...
                                        if (_cfg.Version > 9)
                                        {
                                            ret = _file.GetBlock(out _report, HJS.Block.BlockId.IdReport, false);
                                            if (ret != HJS.ReturnValue.NoError)
                                                _report = null;
                                            ret = mECU.StartUpdateParameterset(ref _cfg, ref _kf, ref _lng1, ref _lng2, ref _lng3, ref _lng4, ref _lng5, ref _lng6, ref _lng7, ref _lng8, ref _report);
                                        }
                                        else
                                        {
        ...
        public void ReadErrorRing(string FileName, int FileExtensionIndex)
        {
            ...
            ErrRingThread.Name = "ReadErrorRing";
            tellProgress(0);
            ErrRingThread.Start();
        }
        ...
        /// <returns>True on success</returns>
        public bool SetOrder(Diag.Orders OrderId, UInt16 OrderValue)
        {
        ...
        /// <returns>True on success</returns>
        public void SaveReadDataFile(string Filename)
        {
            if (!mECU.ReadActualValues())
            {
                tellStatus(String.Format("Could not read values ({0})", mECU.LastReturnValue));
            ...
            else
            {
                ...
                mECU.ErrStack();
                //
                if (!mECU.SaveReadDataFile(Filename))
                {
                    tellStatus(String.Format("Could not save file {0} : {1}", Filename, mECU.LastReturnValue));
                }
                else
                {
                    tellStatus("File saved.");
                }
            }
        }
        ...
        {
            HJS.BlockFile _file = new HJS.BlockFile();
            HJS.Block[] _blk = new HJS.Block[11];
            _blk[0] = mECU.GetBlock(HJS.Block.BlockId.IdKonfig);
            if (_blk[0] == null)
            {
                tellStatus(String.Format("Could not download configuration ({0})", mECU.LastReturnValue));
            }
            else
            {
                _blk[1] = mECU.GetBlock(HJS.Block.BlockId.IdKennfld);
                if (_blk[1] == null)
                {
                    tellStatus(String.Format("Could not download datamaps ({0})", mECU.LastReturnValue));
                }
                else
                {
                    _blk[2] = mECU.GetBlock(HJS.Block.BlockId.IdLngDE);
                    _blk[3] = mECU.GetBlock(HJS.Block.BlockId.IdLngEN);
                    _blk[4] = mECU.GetBlock(HJS.Block.BlockId.IdLngFR);
                    _blk[5] = mECU.GetBlock(HJS.Block.BlockId.IdLngIT);
                    _blk[6] = mECU.GetBlock(HJS.Block.BlockId.IdLngES);
                    _blk[7] = mECU.GetBlock(HJS.Block.BlockId.IdLngNL);
                    _blk[8] = mECU.GetBlock(HJS.Block.BlockId.IdLngPL);
                    _blk[9] = mECU.GetBlock(HJS.Block.BlockId.IdLngTK);
                    _blk[10] = mECU.GetBlock(HJS.Block.BlockId.IdReport);

                    _file.Create(HJS.BlockFile.FileIdentifier.ParameterSet, FileName, true);
                    for (int i = 0; i < _blk.Length; i++)
                    {
                        if (_blk[i] != null) _file.PutBlock(_blk[i]);
                    }
                    _file.Close();
                    tellStatus("Parameterset downloaded!");
                    return true;
                }
            }
            return false;
        ...
        /// <returns>True on success</returns>
        public bool ReadUpdatables()
        {
            byte[] _buffer;
            if (mECU.ReadUpdatables(out _buffer))
            {
                if ((mUpdatables.Length != 10) || (_buffer.Length != 30))
                {
                    tellStatus("Lesefehler SizeMismatch");
                    return false;
                }
                mUpdatables[0] = (BitConverter.ToUInt16(_buffer, 0)).ToString();    //Vert_uiPalarm
                mUpdatables[1] = (BitConverter.ToUInt16(_buffer, 2)).ToString();    //Vert_uiTalarm
                mUpdatables[2] = (BitConverter.ToUInt16(_buffer, 4)).ToString();    //Vert_uiPwarn
                mUpdatables[3] = (BitConverter.ToUInt16(_buffer, 6)).ToString();    //Vert_uiTwarn
                mUpdatables[4] = (BitConverter.ToUInt16(_buffer, 8)).ToString();    //ICDosing_uiKraftstoffverbrauch
                mUpdatables[5] = (BitConverter.ToSingle(_buffer, 10)).ToString();   //Staudruck_fAirMassKFactor
                mUpdatables[6] = (BitConverter.ToSingle(_buffer, 14)).ToString();   //Turnspeed_fFaktorTurns
                mUpdatables[7] = (BitConverter.ToUInt32(_buffer, 18)).ToString();   //Dosieren_ulAdditivTankinhalt
                mUpdatables[8] = (BitConverter.ToUInt32(_buffer, 22)).ToString();   //Additivierung_ulAddStandard
                mUpdatables[9] = (BitConverter.ToSingle(_buffer, 26)).ToString();   //Maf_fDisplacedVolume
                tellStatus("Updatables gelesen");
                return true;
            }
            else
            {
                tellStatus("Lesefehler " + mECU.GetLastReturnValue());
                return false;
            }
        }
        ...
        public string GetUpdatable(int Position)
        {
            if (mUpdatables.Length > Position)
            {
                return mUpdatables[Position];
            }
            else
            {
                return "N/A";
        ...
        /// <returns>True on success</returns>
        public bool SetUpdatable(int Position, byte[] Data)
        {
            if (!mECU.SetUpdatable(Position, Data))
            {
                tellStatus("Schreibfehler " + mECU.GetLastReturnValue());
                return false;
            }
            else
            {
                tellStatus("Wert gesendet");
                return true;
            }
        }
        ...
        public List<string> ReadAvailableLanguages()
        {
            List<string> languages;
            mECU.ReadAvailableLanguages(out languages);
            if (languages == null)
            {
                tellStatus("Lesefehler (SprachenMaske) " + mECU.GetLastReturnValue());
            }
            return languages;
        }
        ...
        public List<string> ReadActiveTaskList()
        {
            List<string> tasks;
            mECU.ReadActiveTaskList(out tasks);
            if (tasks == null)
            {
                tellStatus("Lesefehler (Task Liste) " + mECU.GetLastReturnValue());
            }
            return tasks;
        }
        ...
        public Int16 GetEcuTemperatureOffset()
        {
            return mECU.GetEcuTemperatureOffset();
        }

        /// <summary>Read system basis chip registers</summary>
        /// <returns>Value of SBC registers</returns>
        public UInt16 ReadSbcRegister()
        {
            return mECU.ReadSbcRegister();
        }
    }
}
