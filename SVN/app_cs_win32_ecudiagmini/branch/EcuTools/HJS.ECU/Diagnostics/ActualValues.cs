/*
 * Object: HJS.ECU.Diagnostics.ActualValues
 * Description: Measurement values
 * 
 * $LastChangedDate: 2013-12-17 09:02:02 +0100 (Di, 17 Dez 2013) $
 * $LastChangedRevision: 28 $
 * $LastChangedBy: ksi $
 * $HeadURL: http://menden22/svn/devel/electronic/app_cs_win32_ecudiagmini/branch/EcuTools/HJS.ECU/Diagnostics/ActualValues.cs $
 * 
 * LastReviewDate: 
 * LastReviewRevision: 
 * LastReviewBy: 
...
namespace HJS.ECU.Diagnostics
{
    ...
    public class ActualValues
    {
        ...
        public Block ToBlock()
        {
            if (mReadBuffer != null){
                Block ret = new Block();
                ret.Type = Block.BlockId.IdIstWerte;

                byte[] _raw = new byte[mReadBuffer.Length + 6];
                _raw[(int)Block.HeaderPosition.Type] = (byte)Block.BlockId.IdIstWerte;
                _raw[(int)Block.HeaderPosition.Version] = 1;
                _raw[(int)Block.HeaderPosition.SizeLowByte] = (byte)(mReadBuffer.Length % 256);
                _raw[(int)Block.HeaderPosition.SizeHighByte] = (byte)(mReadBuffer.Length / 256);
                for (int i = 6; i < _raw.Length; i++)
                {
                    _raw[i] = mReadBuffer[i - 6];
                }
