 ...
 * $LastChangedBy: ksi $
 * $HeadURL: http://menden22/svn/devel/electronic/app_cs_win32_ecudiagmini/branch/EcuTools/HJS.ECU/Diagnostics/Diagnostics.cs $
 * 
 * LastReviewDate: 
 * LastReviewRevision: 
 * LastReviewBy: 
...
namespace HJS.ECU.Diagnostics
{
    ...
    public class Diag
    {
        ...
        public Diag(string PortName, byte[] Key, ProtocolBase.LanguageId LanguageIdentifier)
        {
        ...
        {
            return mProtocol.GetEcuTemperatureOffset();
        }
        ...
        /// <param name="Key">Byte array of ECU key. (If not array of 8 bytes, an exception is thrown!)</param>
        public void ChangePassword(byte[] Key)
        {
            if (Key.Length != 8)
            {
                throw new ArgumentException("Diagnostic Keylength invalid");
        ...
        public bool Order(Orders OrderId, UInt16 OrderValue)
        {
            ...
            switch (OrderId)
            {
                ...
                case Orders.calibrateAmbPSensor:
                    ret = mProtocol.SendOrder(HJS.ECU.Port.Comm.OrderByte.StartAmbientPressureSensorCalibration, 0);
                    break;
        ...
        /// <returns>True on success</returns>
        public bool SaveReadDataFile(string Filename)
        {
            ReturnValue ret = ReturnValue.ComOrderFailed;
            BlockFile bf = new BlockFile();
            ret = bf.Create(BlockFile.FileIdentifier.ReadData, Filename, true);
            if (ret == ReturnValue.NoError)
            {
            ...
            if (ret == ReturnValue.NoError)
            {
                if (mActualValues != null)
                {
                    bf.PutBlock(mActualValues.ToBlock());
                }
            }
            if (ret == ReturnValue.NoError)
            {
                bf.PutBlock(mProtocol.GetEmpiricalDataBlock());
            }
            if (ret == ReturnValue.NoError)
            {
                bf.PutBlock(mProtocol.GetVolatileValueBlock());
            }
            if (ret == ReturnValue.NoError)
            {
                bf.PutBlock(mErrorStack.ToBlock());
            }
            bf.Close();
            mLastReturnValue = ret;
            return ret == ReturnValue.NoError ? true : false;
        ...
        public void ReadAvailableLanguages(out List<string> languages)
        {
            byte[] _buffer;
            ReturnValue ret = mProtocol.ReadLanguageMask(out _buffer);
            if (ret == ReturnValue.NoError)
            {
                languages = new List<string>();
                if ((_buffer[0] & 0x01) != 0x00) languages.Add("Deutsch");
                if ((_buffer[0] & 0x02) != 0x00) languages.Add("Englisch");
                if ((_buffer[0] & 0x04) != 0x00) languages.Add("Franzoesisch");
                if ((_buffer[0] & 0x08) != 0x00) languages.Add("Italienisch");
                if ((_buffer[0] & 0x10) != 0x00) languages.Add("Spanisch");
                if ((_buffer[0] & 0x20) != 0x00) languages.Add("Polnisch");
                if ((_buffer[0] & 0x40) != 0x00) languages.Add("Niederlaendisch");
                if ((_buffer[0] & 0x80) != 0x00) languages.Add("Tuerkisch");
            }
            else
            {
                mLastReturnValue = ret;
        ...
        public void ReadActiveTaskList(out List<string> tasks)
        {
            ReturnValue ret = ReturnValue.Retry;
            tasks = new List<string>();
            for (byte i = 0; i < 31; i++)       //31
            {
                ret = mProtocol.IsTaskActive(i);
                if (ret == ReturnValue.NoError)
                {
                    tasks.Add(((HJS.ECU.TaskIdentifier)i).ToString());
                }
                else if(ret != ReturnValue.ComOrderModeFailed)
                {
                    mLastReturnValue = ret;
                }
            }
        ...
        {
            ReturnValue ret = ReturnValue.Retry;
            ret = mProtocol.IsAlive();

            return mProtocol.GetLastChecksum();
        }
                ...
                if (ret == ReturnValue.NoError)
                {
                    ...
                    if (_Report != null)
                    {
                        if (_Report.DataSize != 0)
                        {
                            IstAnzahlBloecke++;
                            //Console.WriteLine(String.Format("Start Downloading {0}/{1}", IstAnzahlBloecke, SollAnzahlBloecke));
                            ret = mProtocol.WriteParamBlock(ref _Report);
                            if (ret != ReturnValue.NoError)
                            {
                                mLastReturnValue = ret;
                                return ret;
        ...
        public Block GetBlock(Block.BlockId Identifier)
        {
            Block ret = new Block();
            ReturnValue r;
            if (Identifier == Block.BlockId.IdReport)
            {
                r = mProtocol.ReadReportBlock(ref ret);
            }
            else
            {
                r = mProtocol.ReadParamBlock(Identifier, ref ret);
            }
            if (r != ReturnValue.NoError)
            {
                mLastReturnValue = r;
                return null;
            }
        ...
        /// <returns>True on success</returns>
        public bool ErrStack()
        {
            return mErrorStack.ReadFromEcu();
        }
        ...
        public UInt64 ErrorActualMask
        {
            get
            {
                return mErrorStack.BitmaskErrorActual;
            }
        }
        ...
        /// <returns>True on success</returns>
        public bool ReadUpdatables(out byte[] target)
        {
            ReturnValue ret = mProtocol.ReadUpdatables(out target);
            mLastReturnValue = ret;
            return ret == ReturnValue.NoError ? true : false;
        }
        ...
        {
            ReturnValue ret = mProtocol.SetUpdatable((UInt16)Position, Data);
            mLastReturnValue = ret;
            return ret == ReturnValue.NoError ? true : false;
        }
        #endregion
