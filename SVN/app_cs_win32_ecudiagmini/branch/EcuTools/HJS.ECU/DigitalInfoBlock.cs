 ...
 * $LastChangedBy: ksi $
 * $HeadURL: http://menden22/svn/devel/electronic/app_cs_win32_ecudiagmini/branch/EcuTools/HJS.ECU/DigitalInfoBlock.cs $
 * 
 * LastReviewDate: 
 * LastReviewRevision: 
 * LastReviewBy: 
    ...
    public class DigitalInfoBlock : Block
    {
        ...
        public ReturnValue Parse(ref byte[] SourceData)
        {
            ...
            if (ret == ReturnValue.BlockNotFound || ret == ReturnValue.VersionMismatch)
            {
                ...
                // generate blockdata to ensure a saveable block
                mBlockData = new byte[DataSize];
                mBlockData[0] = mProductionYear;
                mBlockData[1] = mProductionMonth;
                mBlockData[2] = mProductionDay;
                mBlockData[3] = mProductionHour;
                mBlockData[4] = mProductionMinute;
                mBlockData[5] = mSerialNumber0;
                mBlockData[6] = mSerialNumber1;
                mBlockData[7] = mSerialNumber2;
                mBlockData[8] = mSerialNumber3;
                mBlockData[9] = mHardwareMainLowByte;
                mBlockData[10] = mHardwareMainHighByte;
                mBlockData[11] = mHardwareMidByte;
                mBlockData[12] = mHardwareRevision;
                mBlockData[13] = mSoftwareMainLowByte;
                mBlockData[14] = mSoftwareMainHighByte;
                mBlockData[15] = mSoftwareMidByte;
                mBlockData[16] = mSoftwareRevision;
                mBlockData[17] = mConfigurationMainLowByte;
                mBlockData[18] = mConfigurationMainHighByte;
                mBlockData[19] = mConfigurationMidByte;
                mBlockData[20] = mConfigurationRevision;
                mBlockData[21] = mEcuSecond;
                mBlockData[22] = mEcuMinute;
                mBlockData[23] = mEcuHour;
                mBlockData[24] = mEcuDay;
                mBlockData[25] = mEcuMonth;
                mBlockData[26] = mEcuYear;
                for (int i = 0; i < 10; i++)
                {
                    mBlockData[27 + i] = mAqSource[i];
                }
                mBlockData[37] = mAqSaveMask0;
                mBlockData[38] = mAqSaveMask1;
                mBlockData[39] = mAqSaveMask2;
                mBlockData[40] = mAqSaveMask3;
                mBlockData[41] = scTecuOffset;
                mBlockData[42] = mCompatibleRevision;
                mBlockData[43] = mAqSizeNumBlocks;
                mBlockData[44] = mErrorRingNumSectors;
                mBlockData[45] = mEmpiricalVersion;
                for (int i = 0; i < 3; i++) {
                    for (int j = 0; j < 64; j++)
                    {
                        mBlockData[46 + i * 64 + j] = mEnvironmentFigures[i, j];
                    }
                }
            }
